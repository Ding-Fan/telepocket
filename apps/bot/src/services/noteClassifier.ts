...
// Gemini free tier: 60 RPM, we use conservative 10 RPM
const geminiRateLimiter = createGeminiRateLimiter();
// OpenRouter: 60 RPM with 20-request burst capacity
 ...
 *
 * Rate limiting: 10 requests/minute (conservative for free tier)
...
export class NoteClassifier {
  ...
  private async callOpenRouter(prompt: string): Promise<string> {
    console.log('[OpenRouter] Sending request to model:', config.openrouter.model);
  ...
  private async scoreAllCategories(content: string, urls: string[]): Promise<CategoryScore[]> {
    ...
    if (!config.llm.japaneseCategoryEnabled) {
  ...
  private async scoreSingleCategory(
    ...
    try {
      ...
      console.error(`Error scoring category ${category} with ${this.provider}: ${errorMessage}`);

      // Try fallback to Gemini if OpenRouter failed and fallback is enabled
      if (this.provider === 'openrouter' && config.openrouter.fallbackToGemini && config.gemini.apiKey) {
        try {
  ...
  private getAction(score: number): CategoryScore['action'] {
    if (score >= config.llm.autoConfirmThreshold) {
      return 'auto-confirm';
    }
    if (score >= config.llm.showButtonThreshold) {
      return 'show-button';
    }
    return 'skip';
  }
