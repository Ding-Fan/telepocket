...
export function CreateTagModal({ userId, onClose }: CreateTagModalProps) {
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const [tagName, setTagName] = useState('');
  const [isAITag, setIsAITag] = useState(false);
  const [scorePrompt, setScorePrompt] = useState('');
  const [autoConfirmThreshold, setAutoConfirmThreshold] = useState(95);
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-3xl font-bold text-gray-900">Create New Tag</h2>
            <button
              onClick={onClose}
              ...
              <div className="space-y-6 pl-6 border-l-4 border-blue-200 bg-blue-50/30 py-4 rounded-r-xl">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={`Rate 0-100 how well this note matches "${tagName || '[tag_name]'}".\n\nContent: {content}\nURLs: {urls}\n\nReturn only a number 0-100.`}
