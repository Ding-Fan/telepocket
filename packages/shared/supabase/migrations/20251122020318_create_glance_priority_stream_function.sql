-- Migration: create_glance_priority_stream_function
-- Description: Create function to fetch priority stream glance view - 3 priority notes (marked + high-impression TODOs) + 2 per category
-- Rollback: DROP FUNCTION get_notes_priority_stream(BIGINT, INT, INT);
...
-- Add comment
COMMENT ON FUNCTION get_notes_priority_stream(BIGINT, INT, INT) IS 'Get priority stream glance view - top 3 priority notes (marked + high-impression TODOs) followed by 2 notes per category. Priority notes excluded from category sections.';
