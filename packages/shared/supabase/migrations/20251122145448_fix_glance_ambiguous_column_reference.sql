-- Migration: fix_glance_ambiguous_column_reference
-- Description: Fix ambiguous column reference in NOT IN clause
...
BEGIN
  ...
  category_notes AS (
    ...
    WHERE n.telegram_user_id = telegram_user_id_param
      AND n.status = 'active'
      AND nc.user_confirmed = true
      -- FIX: Use explicit column reference to avoid ambiguity
...
-- Add comment
COMMENT ON FUNCTION get_notes_priority_stream(BIGINT, INT, INT) IS 'Get priority stream glance view - fixed ambiguous column reference and updated_at column issues.';
