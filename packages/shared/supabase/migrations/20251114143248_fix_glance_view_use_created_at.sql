-- Migration: fix_glance_view_use_created_at
-- Description: Fix glance view function to use created_at instead of non-existent updated_at
-- Rollback: Revert to previous version (which was broken)
...
-- Update comment
COMMENT ON FUNCTION get_notes_glance_view IS 'Get glance view of notes - N most recent notes per category, sorted by category recent activity (using created_at)';
